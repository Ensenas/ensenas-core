        ...
        if confidence_score > CONFIDENCE_THRESHOLD:
            ...
            if detection_count[predicted_action] >= DETECTION_THRESHOLD:
                ...
                if last_prediction != predicted_action:
                    print(f"Prediction: {predicted_action}")
...
def corregir_video_stream(data):
    ...
    if not activo:
        if correcto:
            ...
            cv2.putText(img, 'Presione barra espaciadora para reintentar', (10, 170), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2, cv2.LINE_AA)
        else:
            cv2.putText(img, 'Incorrecto', (10, 140), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.putText(img, 'Presione Enter para reintentar', (10, 170), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
